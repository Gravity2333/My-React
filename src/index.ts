...
function Counter() {
  const handleAdd = (num: number) => (Counter.value += num);
  const handleSub = (num: number) => (Counter.value -= num);
  return createElement("div", {}, [
    createElement("h2", {}, ["Counter:", String(Counter.value)]),
    createElement("div", {}, [
      createElement("button", { onClick: handleAdd }, ["+1"]),
      createElement("button", { onClick: handleSub }, ["-1"]),
    ]),
  ]);
}
