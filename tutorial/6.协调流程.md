...
对于Text的处理，其实和SingleElem一样，区别在于，只需要在oldChild中，找到一个类型的TextNode的节点，并且修改其textContent即可，其余的节点就可以删除了。

## reconcileArray 处理多个节点
多节点处理相对复杂一些，其实现逻辑是
对于一个数组的element，需要依次判断每个element和当前所有的oldElement是否可以复用，其复杂度是 O(n^2)的，所以我们需要使用一个Map，把所有的OldElem根据其Key或者Index（顺序） 加入进去，并且遍历新的element数组，每次通过这个Map来匹配key，如果key相同，再匹配type，这样可以降低复杂度到 O(n)!

1. 根据key或者index 创建一个旧Fiber节点的Map 如果没有key，那么就用index 
2. 遍历新的element数组，根据key匹配Map，如果匹配到且type相同 调用useFiber复用
3. 如果没有匹配到，则创建Fiber节点
4. 每次匹配到都删除掉Map中对应的元素
5. 最后Map剩余的元素，需要删除，加入到deletion中
6. 设置 fristNewFiber 和 oldNewFiber 指向第一个FIber元素，后面移动oldNewFiber用来创建sibling
7. lastPlacedIndex = 0 每次创建新的节点， 如果有alternate 查看当前alternate的index是不是比lastPlacedIndex大，如果大就赋给lastPlacedIndex 让其越来越大，如果小，或者不存在alternate 就打上Placement标签。
...
思考一下，为什么key相同的list会让列表项越来越多？

如果old Fiber 存在两个key相同的，那么Map中，只会有第二个Fiber，只会把第二个Fiber复用或者删除。 前面的那个就不会删除。 每次更新旧都多出一个DOM对象来！

为什么key在列表中一定要传， 并且需要和元素本身相关

因为如果不传就会默认用index作为key， 此时如果插入一个元素在中间
原来 1 2 3 4 5
现在 3的位置插入元素
现在INDEX 1 2 3 4 5 6

原本INDEX 1 2 N 3 4 5
那么， 1 2 可以复用 3占用了原来的3  之前的 3 需要用4 的节点，4 需要用5 ...

就导致了插入节点后面的元素都要把属性进行迁移，如果每个节点下都挂着复杂结构，那么低下的内容也需要重新渲染，降低性能！
